...
Our basic proposal is to define a function which quantifies the benefit of a given choice of $n$ and offsets this against the costs of sampling and setup. We'll refer to this as a _value function_, to be distinguished from a utility function in that it can only be applied when all the inputs are known (i.e. it reflects preferences under certainty). We propose two value functions:
...
true_regret <- function(sig, n, lambda, c){
  ...
  reg <- v_opt - v1(n = n, sig = sig, lambda1 = lambda, c1 = c)
...
df <- data.frame(n=seq(4,500,1))
df$sig <- 1
df$v1 <- v1(n = df$n, sig = df$sig, lambda1 = lambda, c1 = C)
...
df$v2 <- v1(n = df$n, sig = df$sig, lambda1 = lambda_new, c1 = C)
